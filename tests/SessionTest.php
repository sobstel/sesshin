...
class SessionTest extends TestCase
{
    ...
    private function setUpDefaultSession($session = null)
    {
        ...
        $emitter = $this
            ->getMockBuilder('\League\Event\Emitter')
            ->setMethods(array('emit', 'addListener'))
            ->getMock();
    ...
    function getSessionMock(array $methods)
    {
        return $this
                ->getMockBuilder('\Sesshin\Session')
                ->setConstructorArgs([$this->getStoreMock()])
                ->setMethods( $methods )
                ->getMock();
    }
    ...
    public function testCreateMethodGeneratesFingerprint()
    {
        $session = $this->setUpDefaultSession(
            $this
                ->getMockBuilder('\Sesshin\Session')
                ->setConstructorArgs([$this->getStoreMock()])
                ->setMethods( array('generateFingerprint'))
                ->getMock()
        );
    ...
    {
        $session = $this->setUpDefaultSession($this->getSessionMock(['create']));
    ...
    {
        $session = $this->setUpDefaultSession($this->getSessionMock(['issetId','create']));
    ...
    {
        $session = $this->setUpDefaultSession($this->getSessionMock(array('create', 'load','isOpened')));
    ...
    {
        $session = $this->setUpDefaultSession($this->getSessionMock(array('isOpened', 'issetId','load')));
        $session->expects($this->any())->method('isOpened')->will($this->returnValue(false));
        $session->getIdHandler()->expects($this->any())->method('issetId')->will($this->returnValue(false));
    ...
    {
        $session = $this->setUpDefaultSession($this->getSessionMock( array('isOpened', 'issetId', 'getFirstTrace','emit') ));
        $session->expects($this->any())->method('isOpened')->will($this->returnValue(false));
        $session->getIdHandler()->expects($this->any())->method('issetId')->will($this->returnValue(true));
    ...
    {
        $session = $this->setUpDefaultSession($this->getSessionMock(array('isOpened', 'issetId', 'getFirstTrace', 'isExpired','emit')));
        $session->expects($this->any())->method('isOpened')->will($this->returnValue(false));
        $session->getIdHandler()->expects($this->any())->method('issetId')->will($this->returnValue(true));
    ...
    {
        $session = $this->setUpDefaultSession($this->getSessionMock( array('getSessionMock','issetId', 'load', 'getFirstTrace', 'isExpired', 'getFingerprint', 'generateFingerprint','isOpened')));
        $session->expects($this->any())->method('isOpened')->will($this->returnValue(false));
        $session->getIdHandler()->expects($this->any())->method('issetId')->will($this->returnValue(true));
        $session->expects($this->once())->method('getFirstTrace')->will($this->returnValue(time()));
    ...
    {
        $session = $this->setUpDefaultSession($this->getSessionMock(array('create','isOpened', 'issetId', 'getFirstTrace', 'isExpired', 'getFingerprint', 'generateFingerprint')));
        $session->expects($this->any())->method('isOpened')->will($this->returnValue(false));
        $session->getIdHandler()->expects($this->any())->method('issetId')->will($this->returnValue(true));
        $session->expects($this->once())->method('getFirstTrace')->will($this->returnValue(time()));
        $session->expects($this->once())->method('isExpired')->will($this->returnValue(false));
    ...
    public function testLoadMethodFetchesDataFromStore()
    {
        ...
        $load_result = $this->invokeMethod($session, 'load');

        $this->assertTrue(true,$load_result);
    }
